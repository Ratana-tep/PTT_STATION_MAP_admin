...
<script>
    ...
    // Initialize all menu toggle functionality
    ...
    function initializeDataTable() {
        ...
        if (table.length === 0) {
            console.error('Stations table not found');
            return;
        }
    ...
    function setupMapModal() {
        ...
        const retryBtn = document.getElementById('retry-load-map');
        const mapsBtn = document.getElementById('open-google-maps');
        
        if (retryBtn) retryBtn.addEventListener('click', handleRetryClick);
        if (mapsBtn) mapsBtn.addEventListener('click', handleOpenInMapsClick);
    }
    ...
    function handleMapsApiError() {
        ...
        showAlert(mapConfig.errorMessages.apiFailed, 'error');
    }
    ...
    function viewLocation(latitude, longitude) {
        try {
            ...
            showAlert(error.message || 'Failed to show location', 'error');
        }
    }
    ...
    function deleteStation(id) {
        if (confirm('Are you sure you want to delete this station?')) {
            if (!id || id === '') {
                showAlert('Invalid station ID', 'error');
                console.error('Invalid ID:', id);
                return;
            }
            
            fetch('https://pttstation.is-best.net/admin/marker-interface.php', {
                method: 'DELETE',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showAlert(data.error || 'Failed to delete station', 'error');
                }
            })
            .catch(error => {
                console.error('Delete error:', error);
                showAlert(`Failed to delete station: ${error.message}`, 'error');
            });
        }
    }
        ...
        fetch(`marker-interface.php?id=${id}`)
            ...
            .then(data => {
                if (data && data.STATION && data.STATION.length > 0) {
                    ...
                    if (currentImageEl) {
                        currentImageEl.innerHTML = station.picture
                    ...
                    ['product', 'other_product', 'description', 'service'].forEach(field => {
                        if (station[field]) {
                            station[field].forEach(value => {
                                const checkbox = document.querySelector(`#edit-station-form input[name="${field}[]"][value="${value}"]`);
                                if (checkbox) checkbox.checked = true;
                            });
                        }
                    });

                    showAlert('Station data loaded for editing.', 'success');
...
after click close model updaet thw web page like be stuck all  whyyy